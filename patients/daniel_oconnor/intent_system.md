...
**Broad open prompts unlock surface facts in one domain.** For example, "tell me about the pain" within the pain-history domain unlocks site, character, onset-timing, and temporal-constancy - not radiation, exacerbating, relieving, severity, or onset-mode (those need specific questions; radiation/exacerbating/relieving are protected).
...
2. **Pain-history surface quartet.** If the pain-history surface set (`pain_site`, `pain_character`, `pain_onset_timing`, `pain_temporal_constancy`) is not yet earned, a broad expansion that pivots to or asks about THE PAIN ("tell me about the chest pain", "describe the pain", "tell me more about the pain", "walk me through the pain") unlocks all four together. The protected pain facts (`pain_radiation`, `pain_exacerbating`, `pain_relieving`) and the specific-only facts (`pain_severity`, `pain_onset_mode`) do NOT unlock on this broad - each requires a direct question.
3. **Associated-symptoms surface (fever only).** If `assoc_fever` is not yet earned, a broad expansion about OTHER SYMPTOMS ("any other symptoms", "anything else been going on", "anything else bothering you", "apart from the chest pain, anything else in your body") unlocks `assoc_fever` only. (No other associated symptoms unlock on broad - the student must ask specifically about malaise, weakness, recent cold/cough, shortness of breath, palpitations, leg signs, immobility, current cough, haemoptysis, wheeze, weight loss, night sweats, etc.)
4. **PMH surface pair.** If neither `pmh_conditions` nor `pmh_surgery` is yet earned, a broad PMH question ("any past medical problems", "any medical history", "any conditions", "anything in your medical history") unlocks both `pmh_conditions` and `pmh_surgery` together as one-time signposting (the patient mentions being generally healthy and signposts the childhood wrist surgery in the same breath). `pmh_cardiac` does NOT unlock on broad PMH - it requires a specific cardiac probe.
5. **Exhaustion.** If the relevant scope-set is already earned (or unavailable), a further broad open unlocks NOTHING. Return `newly_earned: []` with `utterance_type: broad_open`. The patient will deflect naturally and the student must ask something specific.
